 ...
 * 
 * Information flow:
 * How a netbot message happens: in mq2eqbc:: OnPulse()-> Pulse()-> HandleBuffer()-> HandleIncomingString()-> HandleControlMsg()-> SendNetBotEvent() -> pfSendf-> [CROSS HERE] mq2netbots::OnNetBotEVENT()
 *									mq2eqbc:: OnPulse()-> Pulse()-> HandleBuffer()-> HandleIncomingString()-> HandleControlMsg()-> SendNetBotMsg() -> pfSendf -> [CROSS HERE] mq2netbots::OnNetBotMSG() -> Packet.Feed() -> ParseInfo()
 * Things that have to be done to detect adds: Spawns, Structs, passing of information.
 * ToDo:
 * Source of information currently mapped are group, xtargets, anyone in eqbc, and netbots.. but i should really turn off netbots if this is running. why do both when it will info overload.
 * Packet construction needs planned out and unpacked. netbots uses a [NB]|ZoneID^InstanceID|A=%|B=%|...[NB]
 * I feel like it could use the enumerated lists. [SN]|ZoneID^InstanceID|SpawnID|CATEGORIES::OPTION=SUBOPTION=#|[SN]
 * Then can check if GetSpawnByID(Token) if ISNUMBER(Token) and each time will show a new spawn id and recycle the option, sub option, value until all spells are passed.
 enum xTargetTypes
 {
...
enum NETBUFFOPTIONS { SPELL, EXPIRES, CATEGORY, OPTION, AMOUNT };
